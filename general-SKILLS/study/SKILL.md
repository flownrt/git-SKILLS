...
- **It stays provisional.** The map is a working hypothesis, not a contract. When a lesson exposes a missing or misplaced concept, or the goal shifts, revise the curriculum – then re-seed `00-DASHBOARD.html` from it.
...
- **Progress check** – when the user asks "where am I?", map `02-progress/` against the curriculum (if one exists) or the goal's success criteria, and answer at a glance: what's solid, what's shaky, and the single best next step. A snapshot, not a report. Its rendered form is `00-DASHBOARD.html`, which always leads with the overall-progress summary (see [references/dashboard-format.md](./references/dashboard-format.md)): regenerate it whenever you write a progress record, and open it with `present_files` when the user asks where they are.
...
5. **On evidence that reached you** – a question answered in chat, a result reported back, not a quiz the browser graded – write a progress record and clear the pending marker, then update the glossary, references, and (for any subject with a curriculum) its `00-DASHBOARD.html`. A simple wave-off ("I've got it") counts at the depth claimed – they're an adult learner, not an exam candidate.
